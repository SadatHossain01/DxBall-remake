...
void iDraw()
{
    ...
    {
        iShowBMP(0, 0, "assets\\BMP\\bg1.bmp");
        drawPaddle();
        drawBall();
        drawBricks();
        drawPowerups();
        ...
        {
            iShowBMP2(100 + i * 45, 650, "assets\\life.bmp", 0xffffff);
        }
        if (isPaused)
            iShowBMP2(400, 360, "assets\\BMP\\pause thing.bmp", 0x000000);
    }
    else if (CurrentPage == Welcome)
    {
        iShowBMP(0, 0, "assets\\BMP\\welcome.bmp");
    }
    else if (CurrentPage == MainMenu)
    {
        iShowBMP(0, 0, "assets\\BMP\\main-menu.bmp");
    }
    else if (CurrentPage == EnterYourName)
    {
        iShowBMP(0, 0, "assets\\BMP\\Enter Name.bmp");
    ...
    {
        iShowBMP(0, 0, "assets\\BMP\\highscore.bmp");
        iSetColor(0, 255, 255);
        showHighScores();
    }
    else if (CurrentPage == Credits)
    {
        iShowBMP(0, 0, "assets\\BMP\\game_credits.bmp");
    }
    else if (CurrentPage == Instructions)
    {
        iShowBMP(0, 0, "assets\\BMP\\instruction.bmp");
    }
    else if (CurrentPage == Play_New_Continue)
    {
        iShowBMP(0, 0, "assets\\BMP\\new_continue.bmp");
    }
    else if (CurrentPage == Continue_Save_Menu_ExitGame)
    {
        iShowBMP(0, 0, "assets\\BMP\\gameplay_escape.bmp");
    }
    return;
}
...
void iMouse(int button, int state, int mx, int my)
{
    ...
    else if (CurrentPage == MainMenu)
    {
        if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN)
        {
            ...
            else if (mx >= 431 && mx <= 912 && my >= 477 && my <= 526)
            {
                ...
                if (isSoundOn)
                {
                    PlaySound("assets\\music\\Seasons converted.wav", NULL, SND_ASYNC);
                }
                else
                    PlaySound("assets\\music\\completesilence.wav", NULL, SND_ASYNC);
            }
    ...
    {
        if (mx >= 150 && mx <= 150 + 1000 && my >= 400 && my <= 400 + 125)
            isTextBoxActivated = 1;
        else
...
void iPassiveMouseMove(int mx, int my)
{
    if (CurrentPage == Gameplay && !isPaused)
    {
        ...
        if (CurrentPaddle.left < CurrentPaddle.side_circle_radius)
    ...
    if (Ball1.isActivated)
    {
        ...
        if (Ball1.center.x + Ball1.radius >= reference_x && Ball1.center.y - Ball1.radius <= reference_y)
        {
            for (int i = 0; i < total_bricks; i++)
            {
                ...
                if (now->PowerUpAssociation == -1 && isSoundOn)
                    PlaySound("assets\\music\\hitnormalbrick.wav", NULL, SND_ASYNC);
                else
                {
                    if (isSoundOn)
                        PlaySound("assets\\music\\withpowerup.wav", NULL, SND_ASYNC);
                ...
                if (bricks == 0)
                {
                    ...
                    if (isSoundOn)
                        PlaySound("assets\\music\\over.wav", NULL, SND_ASYNC);
                }
                break;
            }
        }
        ...
        if (!shouldRestart && Ball1.center.y - Ball1.radius <= CurrentPaddle.bottom + CurrentPaddle.dy)
        {
            if (Ball1.center.x + Ball1.radius >= CurrentPaddle.left - CurrentPaddle.side_circle_radius && Ball1.center.x - Ball1.radius <= CurrentPaddle.left + CurrentPaddle.dx + CurrentPaddle.side_circle_radius)
            {
                ...
                if (AreBricksFalling)
                    FallBricks();
                if (Ball1.radius < 15.9 && isSoundOn && !AreBricksFalling)
                    PlaySound("assets\\music\\dropnormal.wav", NULL, SND_ASYNC);
                else
                {
                    if (isSoundOn && !AreBricksFalling)
                        PlaySound("assets\\music\\largeballdrop.wav", NULL, SND_ASYNC);
                }
            }
            else
                shouldRestart = 1;
        ...
        if (Ball1.center.x >= window_width - Ball1.radius || Ball1.center.x <= Ball1.radius)
        {
            Velocity1.dx *= -1;
            if (isSoundOn)
                PlaySound("assets\\music\\wallhit.wav", NULL, SND_ASYNC);
        }
        // top wall
        ...
        if (Ball1.center.x <= Ball1.radius)
            Ball1.center.x += (Ball1.radius - Ball1.center.x);
        if (Ball1.center.y >= window_height - Ball1.radius)
            Ball1.center.y -= (Ball1.center.y - window_height + Ball1.radius);
        if (Ball1.center.y <= bottom_margin + Ball1.radius + CurrentPaddle.dy)
        {
            if (!shouldRestart)
                Ball1.center.y += (bottom_margin + Ball1.radius + CurrentPaddle.dy - Ball1.center.y);
        }
        ...
        if (Ball1.center.y + Ball1.radius <= 0)
        {
            reStart();
            if (isSoundOn)
                PlaySound("assets\\music\\balldrop.wav", NULL, SND_ASYNC);
        }
    }
    return;
}
...
void ApplyPowerups(Powerup *this_powerup)
{
    ...
    { // extra life
        if (isSoundOn)
            PlaySound("assets\\music\\positivecaught.wav", NULL, SND_ASYNC);
        if (life < 5)
            life++;
    }
    if (type == 1)
    { // wider paddle
        if (isSoundOn)
            PlaySound("assets\\music\\largethings.wav", NULL, SND_ASYNC);
        if (10 + CurrentPaddle.dx + 2 * CurrentPaddle.side_circle_radius <= window_width / 2)
            CurrentPaddle.dx += 20;
    }
    if (type == 2)
    { // faster ball
        if (isSoundOn)
            PlaySound("assets\\music\\fallbrick.wav", NULL, SND_ASYNC);
    ...
    { // shrink paddle
        if (isSoundOn)
            PlaySound("assets\\music\\shrinkthings.wav", NULL, SND_ASYNC);
        if (CurrentPaddle.dx >= 30)
            CurrentPaddle.dx -= 20;
    }
    if (type == 7)
    { // supershrink paddle
...
const char PowerupImageName[][45] = {"assets\\powerups\\extralife.bmp", "assets\\powerups\\widepaddle.bmp", "assets\\powerups\\fastball.bmp",
                                     "assets\\powerups\\thruball.bmp", "assets\\powerups\\slowball.bmp", "assets\\powerups\\shrinkball.bmp",
                                     "assets\\powerups\\shrinkpaddle.bmp", "assets\\powerups\\supershrinkpaddle.bmp", "assets\\powerups\\megaball.bmp",
                                     "assets\\powerups\\death.bmp", "assets\\powerups\\fallingbricks.bmp"};
void drawPowerups()
{
    for (int i = 0; i < totalpowerups; i++)
    {
        Powerup *this_powerup = &(AllPowerups[i]);
        if (this_powerup->state == 0 || this_powerup->state == 2)
            continue;
...
void HandlePowerups()
{
    for (int i = 0; i < totalpowerups; i++)
    {
        ...
        if (this_powerup->state == 1)
        {
            ...
            if (this_powerup->bottom > CurrentPaddle.bottom + CurrentPaddle.dy)
                continue;
...
void FallBricks()
{
    if (isSoundOn)
        PlaySound("assets\\music\\fallbrick.wav", NULL, SND_ASYNC);
    if (AllBricks[total_bricks - 1].bottom - (CurrentPaddle.bottom + CurrentPaddle.dy + bottom_margin) <= 100)
        return;
    for (int i = 0; i < total_bricks; i++)
    {
...
char digitsInBMP[][25] = {"assets\\digits\\0.bmp", "assets\\digits\\1.bmp", "assets\\digits\\2.bmp", "assets\\digits\\3.bmp", "assets\\digits\\4.bmp", "assets\\digits\\5.bmp", "assets\\digits\\6.bmp", "assets\\digits\\7.bmp", "assets\\digits\\8.bmp", "assets\\digits\\9.bmp"};
